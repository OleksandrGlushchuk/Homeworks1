...
public:
	...
	{
		size = sizeof(VertexType) * num_elements;
		auto vertexBufferDesc = CD3D11_BUFFER_DESC(size, D3D11_BIND_VERTEX_BUFFER, usage, cpuAccessFlags);
	...
	{
		size = sizeof(VertexType) * vertices.size();
		auto vertexBufferDesc = CD3D11_BUFFER_DESC(size, D3D11_BIND_VERTEX_BUFFER, D3D11_USAGE::D3D11_USAGE_IMMUTABLE);
